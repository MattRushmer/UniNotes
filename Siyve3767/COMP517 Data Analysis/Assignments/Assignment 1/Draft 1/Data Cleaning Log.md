...
- **Fix applied:** mean imputation for all six pollutant columns (mean is the statistically correct choice for symmetric data; median would only have been preferred if skew had been meaningfully non-zero) — computed **within each `Country` group** (`df.groupby("Country")[col].mean()`, mapped back onto each row via `df["Country"].map(...)`), not as one dataset-wide number.
