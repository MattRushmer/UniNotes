...
#
# This script goes through the EDA steps from class:
#   1. load the data and take a look at it
#   2. clean it up (missing values, duplicates, outliers)
#   3. explore and visualise the clean data
#   4. look at a few relationships between variables
#
# Everything prints to the console (that's what I used to write the report)
# and all the charts get saved as PNGs in the "figures" folder.
# Run with: python air_quality_eda.py

import os

import matplotlib
matplotlib.use("Agg")  # just save plots to file, don't try to open a window
...
# putting these in lists so I don't have to retype the column names everywhere
...
def savefig(name):
    # helper so every chart gets saved the same way, instead of copy-pasting
    # these lines under every single plot
...
# the messy data has things like "SYDNEY", " Sydney", "sydney " all counted
# as different values, so fix the spacing and capitalisation first
...
# .title() also messes up acronyms like UAE/UK/USA (turns them into
# Uae/Uk/Usa), so fix those ones back manually
...
# the dates are in M/D/YYYY format (you can tell because some values like
# 10/27 only make sense if the first number is the month, not the day)
df["Date"] = pd.to_datetime(df["Date"], format="%m/%d/%Y", errors="coerce")
print("\nRows where Date failed to parse:", df["Date"].isna().sum())


def month_to_quarter(month):
    # using calendar quarters instead of season names (Summer/Winter), since
    # the dataset has countries on both sides of the equator and a season
    # name would be wrong for half of them
    if month in (12, 1, 2):
        return "Q1: Dec-Feb"
    elif month in (3, 4, 5):
        return "Q2: Mar-May"
...
# only the pollutant columns actually have missing values, so loop through
# NUMERIC_COLS and just skip any column that's already complete
...
# filling missing values with each country's own average instead of one
# overall average - otherwise every country would get the exact same
# filled-in number, which hides any real difference between countries
for col in NUMERIC_COLS:
    if df[col].isnull().sum() == 0:
        continue
    skew = df[col].skew()
...
# City and Country don't actually have missing values here, but check anyway
...
# the rows aren't sorted by city/date, so there's no nearby row to guess a
# missing date from - easiest option is just to drop these rows
...
# --- 2e. Outliers ---------------------------------------------------------------
# remove any physically impossible readings first, before working out the
# outlier bounds below (otherwise the bounds would be thrown off)
before_rows = len(df)
...
# also checking PM2.5 with the z-score method since it's the pollutant that
# matters most for health (z-score rule: anything with |z| > 3 is an outlier)
...
# removed - a real pollution spike is exactly the kind of thing we'd want
# to keep in the data, not throw away

# scatter plot showing outliers vs normal points, just for the two
# pollutants people usually care about most
...
for ax in axes.flat[len(NUMERIC_COLS):]:
    ax.axis("off")  # turns off any empty subplot slots that aren't used
...
# turning PM2.5 into a risk category, similar to how BMI gets turned into a
# risk category in the lecture examples. The breakpoints below are from the
# EPA's current PM2.5 AQI table (checked against the EPA site since a lot
# of pages online still show the older, pre-2024 numbers).
...
# turning each risk category into a number from 1-6 with a plain dictionary,
# so it can be averaged and plotted like a normal number
RISK_SCORE = {
    "Good": 1,
    "Moderate": 2,
...
def temperature_band(temp):
    # same idea as month_to_quarter above, just grouping temperature into
    # bands instead of grouping months into quarters
    if temp <= 0:
        return "<0C"
    elif temp <= 10:
        return "0-10C"
